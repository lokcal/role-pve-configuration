# Ansible Role: Adding Ansible User

EDIT KEY PREMISSIONS
...
add ansible-vault to vars/vault
ansible_secvret: ******* # ansible_user
ansible_secvret2: ******* # linux_user

Configures Ansible User on Linux OS for running Playbooks.

# bearbeiten

###### First run without ssh key

ansible_user cant be existend in inventory else it will give connection error because of wrong user

[!TIP]
>[!TIP]
>[!NOTE] callout-info 

´´´ansible-playbook playbooks/add_ansible_user.yml --limit=<HOSTNAME/IP/GROUP> --ask-pass --user=<<SSHUSER> -b --become-method=su --become-user=root -K -vvv

## Requirements

First create ssh-key on local machine an put the Public key in the template folder
...
A list of databases to ensure exist on the server. Only the `name` is required; all other properties are optional.
    ...
    postgresql_users:
      - name: jdoe #required; the rest are optional
        ...
        login_host: # defaults to 'localhost'
        lgin_password: # defaults to not set
        login_user: # defaults to '{{ postgresql_user }}'
        login_unix_socket: # defaults to 1st of postgresql_unix_socket_directories
        port: # defaults to not set
        state: # defaults to 'present'
...
## Example Playbook
    ...
    - hosts: all
      ...
      vars_files:
        - vars/main.yml
      roles:
        - role-ansible-client_configuration
